...
# Code starts here
X_train['int.rate'] = X_train['int.rate'].str.replace('%','').astype('float64')
X_train['int.rate'] = (X_train['int.rate'].values) / 100

X_test['int.rate'] = X_test['int.rate'].str.replace('%','').astype('float64')
X_test['int.rate'] = (X_test['int.rate'].values) / 100
...
# Code starts here
for cols in cat_df :
    X_train[cols].fillna(value = 'NA')
    le = LabelEncoder()
    X_train[cols]= le.fit_transform(X_train[cols])

    X_test[cols].fillna(value = 'NA')
